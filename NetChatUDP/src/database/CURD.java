...
public class CURD {
    ...
    // 记录同一个ip的不同端口在线使用情况
    public int HostPort(String host, String port) {
        Connection conn = ConnectDB();
        try {
            PreparedStatement stmt = null;
            String sql = "insert into t_HostPort (host,port)" +
                    "values(?,?)";
            stmt = conn.prepareStatement(sql);
            stmt.setString(1, String.valueOf(host));
            stmt.setString(2, String.valueOf(port));
            return stmt.executeUpdate();//返回值代表受到影响的行数
    ...
    public Boolean CheckHostPort(String host,String port){
        ...
        try {
            PreparedStatement stmt = null;
            String sql = "select * from t_HostPort where host = ? and port = ?";
            stmt = conn.prepareStatement(sql);
            stmt.setString(1, host);
            stmt.setString(2,port);
            ResultSet rs = stmt.executeQuery();
            if (!rs.next()){//读取数据,如果记录存在
                return false;
            }
            rs.close();
            stmt.close();
    ...
    //删除对应的host和port的对应关系
    public Boolean DelHostPort(String host,String port){
        Connection conn = ConnectDB();
        try {
            PreparedStatement stmt = null;
            String sql = "delete from t_HostPort where host = ? and port = ?";
            stmt = conn.prepareStatement(sql);
            stmt.setString(1, host);
            stmt.setString(2,port);
